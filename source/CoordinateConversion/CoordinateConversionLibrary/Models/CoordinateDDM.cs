...
namespace CoordinateConversionLibrary.Models
{
    public class CoordinateDDM : CoordinateBase
    {
        ...
        public static bool TryParse(string input, out CoordinateDDM ddm)
        {
            ...
            Regex regexDDM = new Regex(@"(?i)^\s*[+]*(?<firstPrefix>[NSEW])?(?<latitudeD>((-| )|(?=\d))\d+)[° ]?(?<latitudeM>\d+[.,:\s]\d+)(?<firstSuffix>[NSEW])?[',\s|/\\]*(?<lastPrefix>[NSEW])?(?<longitudeD>((-| )|(?=\d))\d+)[° ]?(?<longitudeM>\d+[.,:\s]\d+)?[',:\s]*(?<lastSuffix>[NSEW])?");
